...
@export var item_type := "generic"
@export var start_invisible := false

@export var hold_offset := Vector3.ZERO
@export var hold_rotation := Vector3.ZERO

# VISUAL
@export var glow_speed := 3.0
@export var glow_strength := 0.25

# PULSE
@export var pulse_visible_time := 0.5
@export var pulse_hidden_time := 2.0
...
@export var use_empty_icon := true
...
func _process(delta: float) -> void:
	...
	if mesh and mesh.material_override:
		mesh.material_override.emission_energy = (
			0.2 + (sin(glow_time) + 1.0) * 0.5 * glow_strength
		)
...
func _find_ui_icon() -> TextureRect:
	for node in get_tree().get_nodes_in_group("item_ui"):
...
func _update_ui_icon(player: PlayerController) -> void:
	...
	elif use_empty_icon and empty_icon_texture:
