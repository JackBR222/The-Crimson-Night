extends StaticBody3D
class_name InteractiveActor

# TIMELINES
@export var dialog_timeline_1: Resource
@export var dialog_timeline_2: Resource
@export var dialog_timeline_3: Resource
@export var dialog_timeline_4: Resource

# MODELOS
@export var model_1: Node3D
@export var model_2: Node3D
@export var model_3: Node3D
@export var model_4: Node3D

# CENAS
@export_file("*.tscn", "*.scn") var next_scene_path := ""

# DATA
@export var actor_id := "A"

# COMEÇA BLOQUEADO
@export var start_locked := false

@onready var icon: Sprite3D = $InteractionIcon

var current_timeline: Resource
var pending_timeline: Resource

var pending_scene_path := ""

var current_player: PlayerController

var interaction_locked := false
var is_targeted := false

func _ready() -> void:
	current_timeline = dialog_timeline_1
	interaction_locked = start_locked

	_apply_model(1)

	if icon:
		icon.visible = false

	Dialogic.signal_event.connect(_on_dialogic_signal)

# CONTROLE DO ÍCONE
func set_targeted(state: bool) -> void:
	is_targeted = state
	_update_icon()

func _update_icon() -> void:
	if icon:
		icon.visible = (
			is_targeted
			and not interaction_locked
			and current_player == null
		)

# INTERAÇÃO
func interact(player: Node) -> void:
	if interaction_locked or not player is PlayerController:
		return

	current_player = player

	set_targeted(false)

	_update_dialogic_context()
	start_dialog()

func start_dialog() -> void:
	if current_player:
		current_player.freeze_input()

	# congela inimigos
	get_tree().call_group(
		"enemies",
		"set_process_mode",
		Node.PROCESS_MODE_DISABLED
	)

	_disconnect_dialogic()

	Dialogic.timeline_ended.connect(_on_timeline_ended)
	Dialogic.start(current_timeline)

# FIM DO DIÁLOGO
func _on_timeline_ended() -> void:
	_disconnect_dialogic()

	# descongela inimigos
	get_tree().call_group(
		"enemies",
		"set_process_mode",
		Node.PROCESS_MODE_INHERIT
	)

	if current_player:
		current_player.unfreeze_input()
		current_player = null

	Dialogic.VAR.set("current_door_id", "NONE")

	if pending_timeline:
		current_timeline = pending_timeline
		pending_timeline = null

	is_targeted = true

	_update_icon()

	if pending_scene_path != "":
		var scene_to_load := pending_scene_path

		pending_scene_path = ""

		get_tree().change_scene_to_file(scene_to_load)

# SINAIS DO DIALOGIC
func _on_dialogic_signal(argument: String) -> void:

	if argument.begins_with("checkpoint_"):
		Checkpoint.definir_checkpoint(
			int(argument.replace("checkpoint_", ""))
		)

		return

	if (
		argument != "checkpoint_test"
		and Dialogic.VAR.get("current_door_id") != actor_id
	):
		return

	match argument:

		"lock_interaction":
			lock_interaction()

		# TIMELINES
		"change_timeline_1":
			change_timeline(dialog_timeline_1)

		"change_timeline_2":
			change_timeline(dialog_timeline_2)

		"change_timeline_3":
			change_timeline(dialog_timeline_3)

		"change_timeline_4":
			change_timeline(dialog_timeline_4)

		# MODELOS
		"change_model_1":
			_apply_model(1)

		"change_model_2":
			_apply_model(2)

		"change_model_3":
			_apply_model(3)

		"change_model_4":
			_apply_model(4)

		"checkpoint_test":
			Checkpoint.definir_checkpoint(-1)

		"change_scene":
			change_scene(next_scene_path)

# CONTROLE DE TIMELINE
func change_timeline(new_timeline: Resource) -> void:
	pending_timeline = new_timeline

# CONTROLE DE CENA
func change_scene(scene_path: String) -> void:
	pending_scene_path = scene_path

# CONTROLE DE MODELO
func change_model(model_index: int) -> void:
	_apply_model(model_index)

func _apply_model(model_index: int) -> void:

	var models := [
		model_1,
		model_2,
		model_3,
		model_4
	]

	for i in range(models.size()):

		var model = models[i]

		if model:
			var enabled := (i + 1) == model_index

			model.visible = enabled
			_set_physics_enabled(model, enabled)

func _set_physics_enabled(root: Node, enabled: bool) -> void:
	for child in root.get_children():

		if child is CollisionShape3D:
			child.disabled = not enabled

		elif child is CollisionObject3D:
			child.set_deferred("disabled", not enabled)

		if child.get_child_count() > 0:
			_set_physics_enabled(child, enabled)

# BLOQUEIO DE INTERAÇÃO
func lock_interaction() -> void:
	interaction_locked = true

	if current_player:
		current_player.unfreeze_input()
		current_player = null

	_update_icon()

	print("Interação bloqueada: ", actor_id)

# CONTEXTO DO DIALOGIC
func _update_dialogic_context() -> void:
	Dialogic.VAR.set("current_door_id", actor_id)

# HELPERS
func _disconnect_dialogic() -> void:
	if Dialogic.timeline_ended.is_connected(_on_timeline_ended):
		Dialogic.timeline_ended.disconnect(_on_timeline_ended)
